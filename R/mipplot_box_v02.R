...
mipplot_box <- function(D,region=levels(D$region),variable=levels(D$variable), target_year=levels(D$period),PRINT_OUT=F,DEBUG=T){
 ...
 for(r in levels(as.factor(region))){
   ...
   for (v in levels(as.factor(variable))){
     ...
     for(ty in levels(as.factor(target_year))){
       ...
       p_Out1 <- ggplot2::ggplot(D_sub, ggplot2::aes(x=scenario, y=value))+
