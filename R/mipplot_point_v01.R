...
mipplot_point <- function(
    ...
    for (v in levels(as.factor(variable))) {
      ...
      for (ty in levels(as.factor(target_year))) {
        ...
        p_Out1 <- ggplot2::ggplot(
          data = D_sub,
          ggplot2::aes(x = scenario, y = value)) +
          ggplot2::geom_point(
            ggplot2::aes(color = model, shape = model), size = 5) +
          ggplot2::labs(
            title = tt1, subtitle = tt2, y = tt3) +
          ggplot2::theme(
  ...
  if (PRINT_OUT == TRUE) {
    ...
    filename <- sprintf(
      "../data_output/JpMIP_plots_point_%s.pdf",
