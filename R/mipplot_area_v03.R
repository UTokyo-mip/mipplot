...
#' @description Area plots using right-hand-side values of target additivity rule.
#' The function arguments inlcude the input dataframe, labels for the plot/axes/legend, and faceting dimensions (two in this version).
...
mipplot_area <- function(D, R, region=levels(D$region), scenario=levels(D$scenario), facet_x=NULL, facet_y=NULL, PRINT_OUT=F, DEBUG=T,fontsize=20){

  #REPLACED THIS FUNCTION WITH 1-LINE CODE (SEE LINE 53).
  ...
  for (i in levels(as.factor(R$Rule_ID))){
    ...
    #for(r in c("ASIA")){  ## For testing

      for(s in levels(as.factor(scenario))){
      #for(s in c("EMF27-450-Conv")){  ## For testing

        Var_set <- R[R$Rule_ID==i,]


        ## SELECT DATA
        D_LHS <- D[D$region==r & D$scenario==s & D$variable %in% Var_set$Left_side, ]
        D_RHS <- D[D$region==r & D$scenario==s & D$variable %in% Var_set$Right_side, ]
    ...
    if (PRINT_OUT == TRUE){
      ...
      filename <-sprintf("JpMIP_plots_area_%s.pdf", format(Sys.time(), "%Y_%m%d"))
