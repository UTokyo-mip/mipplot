...
mipplot_bar_test <- function(
  D, R,region = levels(D$region),variable = levels(D$variable),
  scenario = levels(D$scenario), model = levels(D$model),
  ...
  R1 <- R[R$Left_side %in% variable, ]
  R <- R[R$Rule_ID %in% R1$Rule_ID,]
  ...
  for (i in levels(as.factor(R$Rule_ID))) {
    ...
    for (r in levels(as.factor(region))) {
      ...
      for (ty in levels(as.factor(target_year))) {
        ...
        D_RHS$variable <- str_replace_all(D_RHS$variable,fixed("|")," ")
  ...
  if (PRINT_OUT == TRUE){
    ...
    getwd()
    filename <- sprintf(
      "../R/data_output/mipplot_plots_bar_%s.pdf",
